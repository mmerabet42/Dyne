...
dn::Camera::Camera(const float &p_fov, const float &p_near, const float &p_far, const float &p_aspectRatio)
	: Component("Camera"),  _fov(p_fov), _far(p_far), _near(p_near), _aspectRatio(p_aspectRatio)
{
	// The last created camera becomes the main camera
	dn::Camera::main = this;
}
...
// The camera's start and update functions do nothing,
// but they must declared in order for the component to be attachable
void dn::Camera::start() {}
void dn::Camera::update() {}

// By default there is no main camera, the last created camera becomes the main camera
